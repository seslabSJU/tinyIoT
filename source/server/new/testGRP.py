...
class TestGRP(unittest.TestCase):
	...
	def _createContainers(cls) -> None:
		...
		cls.cnt1, rsc = CREATE(aeURL, TestGRP.originator, T.CNT, dct)
		assert rsc == RC.CREATED, 'cannot create container'
		cls.cnt1RI = findXPath(cls.cnt1, 'm2m:cnt/ri')
		dct = 	{ 'm2m:cnt' : { 
					'rn'  : f'{cntRN}2'
				}}
		cls.cnt2, rsc = CREATE(aeURL, TestGRP.originator, T.CNT, dct)
		assert rsc == RC.CREATED, 'cannot create container'
	...
	def test_addCNTtoGRP(self) -> None:
		...
		self.cnt3, rsc = CREATE(aeURL, TestGRP.originator, T.CNT, dct)
		self.assertEqual(rsc, RC.CREATED)
	...
	def test_addDeleteContainerCheckMID(self) -> None:
		...
		self.cnt4, rsc = CREATE(aeURL, TestGRP.originator, T.CNT, dct)
		...
		# Delete container
		r, rsc = DELETE(f'{aeURL}/{cntRN}4', TestGRP.originator)
		self.assertEqual(rsc, RC.DELETED)
