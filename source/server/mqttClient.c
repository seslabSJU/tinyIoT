                           ...
                           byte msg_new, byte msg_done)
{
    ...
    if (pjson->valuestring)
        o2pt->fr = strdup(pjson->valuestring);
...
static int mqtt_tls_verify_cb(int preverify, WOLFSSL_X509_STORE_CTX *store)
{
    ...
    PRINTF("MQTT TLS Verify Callback: PreVerify %d, Error %d (%s)",
           preverify, store->error, store->error != 0 ? wolfSSL_ERR_error_string(store->error, buffer) : "none");
...
int mqtt_forwarding(oneM2MPrimitive *o2pt, char *host, int port, cJSON *csr)
{
    ...
    else
    {
        ...
        rc = MqttClient_NetConnect(&notiClient, host, port,
                                   MQTT_CON_TIMEOUT_MS, MQTT_USE_TLS, mqtt_tls_cb);
        if (rc != MQTT_CODE_SUCCESS)
        {
            goto exit;
