...
bool isFCAttrValid(cJSON *fc)
{
    ...
    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "sts")) < 0)
        return false;
    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "stb")) < 0)
        return false;

    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "sza")) < 0)
        return false;
    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "szb")) < 0)
        return false;

    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "lim")) < 0)
        return false;

    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "lvl")) < 0)
        return false;
    if (cJSON_GetNumberValue(cJSON_GetObjectItem(fc, "ofst")) < 0)
        return false;
...
int validate_filter_criteria(oneM2MPrimitive *o2pt)
{
    ...
    if (!isFCAttrValid(fc))
    { // TODO - If rcn == 11(discovery result references) fu must be 1
...
bool isResourceAptFC(oneM2MPrimitive *o2pt, RTNode *rtnode, cJSON *fc)
{
    ...
    if (pjson && pjson2)
    {
        if (strcmp(pjson->valuestring, pjson2->valuestring) >= 0 && fo == FO_AND)
            return false;
    }
    if (pjson)
    {
    ...
    if (pjson && pjson2)
    {
        if (pjson->valueint >= pjson2->valueint && fo == FO_AND)
            return false;
    }
